...
public:
    ...
    // With this → safe even if parameter and member variable have same name.
...
private:
    ...
    double cap;
public:
    PercentageWithCapStrategy(double amount, double cap) {
        this->amount = amount;
        this->cap = cap;
    }
    double calculate(double baseAmount) override {
        double disc = baseAmount * (amount / 100.0);
        if (disc > cap) {
            return cap;
        }
        return disc;
...
private:
    ...
    // this is private constructor It prevents anyone outside the class from creating an object
    // directly using new DiscountStrategyManager() or on the stack.
    DiscountStrategyManager() {}
    // These delete the copy constructor and assignment operator.
